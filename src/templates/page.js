    ...
    <div className="card">
      <div className="card-body pl-0">
        <h5 className="card-title">
          {card.node.link ? (
            <a href={card.node.link}>{card.node.title}</a>
          ) : (
            card.node.title
          )}
        </h5>
        <div className="card-text">
          <ReactMarkdown source={card.node.text} />
        </div>
      </div>
    </div>
  </div>
...
const toCards = (cards, tag) =>
  (cards ? cards.edges : [])
    ...
    .map(card => (
      <Card key={card.node.title.split(" ").join("-")} card={card} />
    ));
...
const Page = ({ data, pageContext }) => {
  ...
  return (
    <Layout menu={pageContext.siteNav}>
      <SEO
        title={frontmatter.title || frontmatter.hiddenTitle}
        lang={frontmatter.lang || "it"}
      />

      <div className={getLayoutClasses(frontmatter)}>
        {frontmatter.title && <h1>{frontmatter.title}</h1>}
        <div dangerouslySetInnerHTML={{ __html: html }} />
        {frontmatter.link_consulta_obiettivi && (
          <p>
            <a
              className="primary-color"
              href={frontmatter.link_consulta_obiettivi}
            >
              consulta obiettivi Piano Triennale 2019-2021
            </a>
          </p>
        )}
      </div>
        ...
        <div className="my-4">
          {done[0] && (
            <>
              ...
              <div className="d-flex flex-wrap">{done}</div>
              {frontmatter.link_approfondisci && (
                <div className="d-flex flex-wrap">
                  <p className="mt-4 ml-auto text-right">
                    <a
                      className="primary-color"
                      href={frontmatter.link_approfondisci}
                    >
          ...
          {todo[0] && (
            <div className="my-5">
              ...
              <div className="d-flex flex-wrap">{todo}</div>
              {frontmatter.link_visualizza_azioni && (
                <div className="d-flex flex-wrap">
                  <p className="mt-4 ml-auto text-right">
                    <a
                      className="primary-color"
                      href={frontmatter.link_visualizza_azioni}
                    >
                      Visualizza azioni nel Piano Triennale 2019-2021
                    </a>
                  </p>
                </div>
              )}
      ...
      {pageNav && (
        <PageNav
          left={{
            path:
              frontmatter.link_prev !== null
                ? frontmatter.link_prev
                : pageNav.prev.path,
            label: frontmatter.link_prev_label || pageNav.prev.name
          }}
          down={{
            path:
              frontmatter.link_linee_azione !== null
                ? frontmatter.link_linee_azione
                : pageNav.firstChild.path,
            label: frontmatter.link_linee_azione
              ? "Confronto per linee d'azione"
              : pageNav.firstChild.name
          }}
          /* up={{
          ...
          right={{
            path:
              frontmatter.link_next !== null
                ? frontmatter.link_next
                : pageNav.next.path,
